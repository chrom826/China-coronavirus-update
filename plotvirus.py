...
cur.execute('''SELECT confirm FROM coronavirus WHERE longitude IS NOT NULL''')
confirmlist = cur.fetchall()
confirm = [float(x[0]) for x in confirmlist]


confirmmap = Geo("Confirmed case by 14-02-2020", width=1000, height=800)
#confirmmap.use_theme('dark')
for i in range(len(name)):
	confirmmap.add_coordinate(name[i],lng[i],lat[i])
	n = [name[i]]
	c = [confirm[i]]
	confirmmap.add("confirmed case",n,c, maptype='china',
             is_visualmap=True, 
             symbol_size=1.5*np.power(c*2,0.4),
             border_color = '#fff')
confirmmap.render('14-02-2020.html')